...
from imports.colors import red, green, blue, bold
...
class Controller:
    ...
    def __init__(self):
        ...
        self.commandsWithoutDescription = {'search': '', 'list all': '',
                                           'use': '', 'info': '',
                                           'get': '', 'report-mal': '',
                                           'update-db': '', 'help': '',
                                           'exit': ''}

        self.searchmeth = [("arch",
                            "which architecture etc; x86, x64, arm7 so on..."),
                           ("plat",
        ...
        while (True):  # Dont hate, affiliate
            try:
                if self.currentmodule is not None:
                    just_print = self.db.query(
                        "SELECT NAME FROM Malwares WHERE ID=?",
                        self.currentmodule)[0][0]
                    cmd = raw_input(
                        bold(green('mdb ')) +
                        bold(blue(just_print)) + green('#> ')).strip()
                else:
                    cmd = raw_input(
    ...
    def actOnCommand(self, cmd):
        try:
            ...
            if cmd == 'report-mal':
                ...
                print("To avoid compromising your privacy we" +
                      " have chose this method of reporting.")
                print("If you have not stated your name we will" +
                      " not write a thanks in our README.")
                print("Your email will remain private in scenario" +
                      " and will not be published.")
                print("")
                print("Please create an archive file with the structure" +
            ...
            if cmd == 'info':
                ...
                manySearch = manysearches.MuchSearch()
                manySearch.print_payloads(
                    m,
                    [
                        "%", "Name", "Ver.", "Author", "Lang",
                        "Date", "Arch.", "Plat.", "Tags"
                    ])
                return
