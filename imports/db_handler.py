...
class DBHandler:
    ...
    def get_partial_details(self):
        return self.cur.execute("""SELECT
            ...
            FROM Malwares""").fetchall()

    def get_mal_list(self):
        return self.cur.execute(
            "SELECT ID, NAME, TYPE From Malwares"
            ).fetchall()
    ...
    def get_mal_names(self):
        ...
        return [val[0] for val in self.cur.execute(
            "SELECT NAME FROM Malwares"
            ).fetchall()
            ]

    def get_mal_tags(self):
        return [val[0] for val in self.cur.execute(
            "SELECT DISTINCT TAGS From Malwares WHERE TAGS IS NOT NULL"
            ).fetchall()
            ]
    ...
    def get_mal_info(self, mid):
        return self.cur.execute(
            ...
            From Malwares WHERE ID =""" + str(mid)
        ).fetchall()
    ...
    def query(self, query, param=''):
        ...
        try:
            if param is not '':
                return self.cur.execute(
                    query,
                    param if type(param) is list else [param]
                ).fetchall()
            else:
